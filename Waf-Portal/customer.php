...
<title>Indusface Dashboard</title>
 <?php include("./include/header5.php");?>
...
margin:0 0 0 700px;
 ...
 <div class="container1">
	...
	<h4><div class="option">
						    ...
						    <li id="export-to-excel"><a href="reports_backup/xls/report_<?php echo $_POST['search_customer'];?>_xls.xls">Export to EXCEL</a></li>
							<li id="export-to-csv"><a href="reports_backup/csv/report_<?php echo $_POST['search_customer'];?>_csv.csv">Export to CSV</a></li>
...
<?php
	...
	if (!$link = mysql_connect("54.224.77.103","yash1","ycHyyPmfGtVxRrYa")) {
        ...
        if (!$rs = mysql_query("SELECT CustomerID,AlertID,FoundDate,Description,URL,Method,Para,ReqHeader,Replay_TIME,HTTP_RESPONSE_CODE,RawLog,Rule,Unique_ID FROM SigReplayStatus $searchString  AND FoundDate LIKE '{$_POST['date']}%'")) {
          ...
          $count=0;
		  $excel=new ExcelWriter("reports_backup/xls/report_{$_POST['search_customer']}_xls.xls");
...
$excel->writeCol(htmlentities($row[12]));
	...
	$data = $inspector;
	$buffer = file_get_contents("export.php");
	
	}
	$fp = fopen("reports_backup/csv/report_{$_POST['search_customer']}_csv.csv", 'w');
	fputcsv($fp,$myArr);
